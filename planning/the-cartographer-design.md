...
- **Isolated settlements** get the same nearest-neighbour link, so every location stays reachable.
...
| Tests     | A small committed Azgaar fixture drives parser, mapper, and loader tests; MCP tools tested like Scriptorium (a real client over express); Storage and Firestore via emulators                      |

---

## 6. Open Questions

- **Settlement detail.** Large maps have thousands of settlements. **Recommendation:** import all of them and decide on filtering (for example, skipping hamlets under a population threshold) after trying play with a large map.
