...
- **Empty or shared names.** An empty name gets a fallback. Names are unique across locations and factions, because the Loom resolves "go to X" by name. A name shared by several settlements is qualified by province, or by realm when the province carries the town's own name ("Norton (Hertfield)", "Bramburgh (Torkland)"). A shared point-of-interest name is qualified by its nearest settlement ("Pirates near Coliverlis"). A number is added only if both still collide. Nisia has 54 such names.
