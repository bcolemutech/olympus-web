        ...
        {manual && (
          <div style={{ display: "flex", gap: 5 }}>
            {[["−", "#c43a3a", onRemove], ["+", "#3ac45a", onAdd], ["↺", "#3a8fc4", onResetUsed]].map(([lbl, clr, fn]) => (
              <button key={lbl} type="button" onClick={fn} style={{ background: "none", border: `1px solid ${clr}`, color: clr, borderRadius: 6, width: 28, height: 28, cursor: "pointer", fontSize: 14 }}>{lbl}</button>
            ))}
          </div>
        )}
      </div>
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#fff", fontFamily: "system-ui, sans-serif", padding: "16px 14px", maxWidth: 700, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
        {[["Match Counter", showMatch, setShowMatch], ["Ball Counter (14.1)", showBall, setShowBall], ["Manual Mode", manual, setManual]].map(([lbl, on, set]) => (
          <button key={lbl} type="button" onClick={() => set(v => !v)} style={{
      ...
      <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 12, padding: 18, marginBottom: 18 }}>
        ...
        <button type="button" onClick={deal} style={{ width: "100%", background: "#1a3a1a", border: "1px solid #3ac45a", color: "#3ac45a", borderRadius: 8, padding: "11px 0", fontWeight: 800, fontSize: 14, letterSpacing: 2, cursor: "pointer", marginTop: 4 }}>
          🃏 DEAL HANDS
        </button>
      </div>
